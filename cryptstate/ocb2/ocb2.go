...
func zeros(block []byte) {
	...
	//	block[i] = 0
	//}
	bw := (*[2]uintptr)(unsafe.Pointer(&block[0]))
	bw[0] = 0
	bw[1] = 0
}
...
func Encrypt(cipher cipher.Block, dst []byte, src []byte, nonce []byte, tag []byte) {
	...
	checksumPtr := bufPool.Get().(*[]byte)
	deltaPtr := bufPool.Get().(*[]byte)
	tmpPtr := bufPool.Get().(*[]byte)
	padPtr := bufPool.Get().(*[]byte)
	calcTagPtr := bufPool.Get().(*[]byte)

	defer func() {
		bufPool.Put(checksumPtr)
		bufPool.Put(deltaPtr)
		bufPool.Put(tmpPtr)
		bufPool.Put(padPtr)
		bufPool.Put(calcTagPtr)
	}()

	checksum := (*checksumPtr)[0:BlockSize]
	delta := (*deltaPtr)[0:BlockSize]
	tmp := (*tmpPtr)[0:BlockSize]
	pad := (*padPtr)[0:BlockSize]
	calcTag := (*calcTagPtr)[0:TagSize]
...
func Decrypt(cipher cipher.Block, plain []byte, encrypted []byte, nonce []byte, tag []byte) bool {
	...
	tmp := (*tmpPtr)[0:BlockSize]
	pad := (*padPtr)[0:BlockSize]
	calcTag := (*calcTagPtr)[0:NonceSize]
